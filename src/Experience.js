...
export default function Experience() {
  ...
  const Nucleus = ({ protonCount, neutronCount }) => {
    ...
    const distributeParticles = (particleCount, isProton) => {
      ...
      const radius =
        Math.max(protonCount, neutronCount) <= 20
          ? Math.max(protonCount, neutronCount) * 0.02
          : Math.max(protonCount, neutronCount) * 0.01; // Adjust the multiplier as needed
      const goldenAngle = Math.PI * (3 - Math.sqrt(5));

      for (let i = 0; i < particleCount; i++) {
        const theta = goldenAngle * i;
        const y = isProton
          ? ((i / particleCount) * 2 - 1) * radius * 0.7
          : ((i / particleCount) * 2 - 1) * radius;
  ...
  return (
    <>
      ...
      <group scale={5}>
        {/* Proton & Neutron Count to be passed here  */}
